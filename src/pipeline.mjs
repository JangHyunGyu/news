...
export async function ensureTranslationSchema(env) {
  for (const column of ['explanation TEXT', 'original_content TEXT', "translation_status TEXT DEFAULT 'legacy'", 'translation_model TEXT']) {
...
export async function prepareNews(stories, complete, fetchImpl = fetch) {
  ...
  async function run() {
    while (next < stories.length) {
      ...
      let source = '';
      try {
        source = await fetchArticleContent(story, fetchImpl);
        ...
        results[index] = {
          ...
          original_content: source, translation_status: source ? 'failed' : 'unavailable', models: '',
        };
      }
    }
  }
  ...
  for (let i = 0; i < stories.length; i++) {
    ...
    if (refresh) {
      ...
      statements.push(env.DB.prepare(`UPDATE news SET translated_title = ?, summary = ?, explanation = ?,
        original_content = ?, translation_status = ?, translation_model = ? WHERE date = ? AND hn_id = ?
        ...
        translated.translation_status, translated.models, date, story.id, translated.translation_status,
      ));
    } else {
      statements.push(env.DB.prepare(`INSERT INTO news
        (hn_id, date, rank, original_title, translated_title, summary, explanation, url, score, original_content, translation_status, translation_model)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(
        ...
        translated.original_content, translated.translation_status, translated.models,
      ));
    }
  }
