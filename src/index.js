...
- 원문의 문단·소제목·인용·목록·말투와 주장의 강도를 유지하되, 독자가 이해하는 데 필요한 쉬운 설명을 덧붙입니다. 소스 코드·명령어·자료형 이름은 그대로 나열하지 말고, 그 코드가 기사에서 하는 일을 쉬운 말로 설명합니다.
- 독자가 서버·브라우저·암호화·오픈소스 같은 용어도 모를 수 있다고 생각합니다. 전문용어는 처음 나올 때 쉬운 뜻과 쓰임을 설명하고, 다른 전문용어만으로 정의하지 않습니다. 약어의 영어 이름만 늘어놓지 않습니다. 일상에서 쓰는 말로 바꿀 수 있으면 바꿉니다.
...
- 날짜·시간·비용·비율·비교 방향과 조건은 원문과 대조합니다. 짧은 시간과 긴 시간의 범위를 뒤집거나 서로 다른 실험의 수치를 합치지 않습니다. 원문이 추정한 내용은 추정으로 남깁니다.
- 비율을 쓸 때는 무엇 중 몇 퍼센트인지 대상 범위를 함께 밝힙니다. 특정 서비스의 고객·주소·장비 일부에 관한 수치를 전체 인터넷이나 모든 이용자로 확대하지 않습니다.
- 일부 사례를 모든 서비스의 안전성이나 위험으로 일반화하지 않습니다. '철통보안', '시사하는 바가 큽니다' 같은 과장·상투어를 쓰지 말고 어떤 차이가 있는지 구체적으로 설명합니다.
